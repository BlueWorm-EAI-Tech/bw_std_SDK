...
下图展示机器人处于关节零位时的坐标轴方向。当前位置下，双臂和头部的
电机均处在关节零位。

![Standard 机器人坐标系与关节零位示意图](./docs/images/robot_coordinate_system.png)

所有旋转关节均在各自的局部坐标系中遵循右手定则，绕局部坐标轴的正向
旋转取为正值。公共 API 的关节顺序与轴向约定如下：

**双臂关节轴序（`Degree1 ~ Degree7`）**

| 关节 | `Degree1` | `Degree2` | `Degree3` | `Degree4` | `Degree5` | `Degree6` | `Degree7` |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 局部旋转轴 | `+Y` | `+X` | `+Z` | `+Y` | `+X` | `+Y` | `+Z` |
...
上述方向用于解释目标角度的正负号；左右两侧手臂均按各自局部坐标系
应用同一右手定则。机器人端会根据具体执行机构完成坐标变换，SDK 客户端
不需要额外反转角度或自行应用镜像规则。
