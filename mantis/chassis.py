...
class Chassis:
    ...
    def set_friction(self, linear: float = None, angular: float = None):
        ...
        if linear is not None:
            self._linear_friction = max(0.5, min(5.0, abs(finite_float(linear, "linear"))))
        if angular is not None:
            self._angular_friction = max(0.5, min(5.0, abs(finite_float(angular, "angular"))))
    ...
    def set_default_speed(self, linear: float = None, angular: float = None):
        ...
        """
        if linear is not None:
            self._default_linear_speed = max(
                0.01,
                min(self.MAX_LINEAR_SPEED, abs(finite_float(linear, "linear"))),
            )
        if angular is not None:
            self._default_angular_speed = max(
                0.1,
                min(self.MAX_ANGULAR_SPEED, abs(finite_float(angular, "angular"))),
            )
    ...
    def _move_linear(self, dx: float, dy: float, speed: float = None, block: bool = True):
        ...
        Args:
            ...
            block: 是否阻塞
        """
        dx = finite_float(dx, "dx")
        dy = finite_float(dy, "dy")
        distance = math.sqrt(dx * dx + dy * dy)
        if distance < 0.001:  # 距离太小，忽略
            return
        
        # 计算速度
        spd = finite_float(speed, "speed") if speed is not None else self._default_linear_speed
    ...
    def _rotate(self, degrees: float, speed: float = None, block: bool = True):
        ...
        Args:
            ...
            block: 是否阻塞
        """
        degrees = finite_float(degrees, "degrees")
        radians = math.radians(degrees)
        if abs(radians) < 0.001:  # 角度太小，忽略
            return
        
        # 计算速度
        spd = finite_float(speed, "speed") if speed is not None else self._default_angular_speed
